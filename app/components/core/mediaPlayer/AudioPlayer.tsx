...
export default function AudioPlayer({ src, duration, onComponentReady = () => {} }: AudioPlayerProps) {
  ...
  useEffect(() => {
    ...
    const handlePlay = () => {
      ...
      if (effectiveDuration != null && audio.currentTime >= effectiveDuration - 0.1) {
        audio.currentTime = 0;
      }
    };
  ...
  return (
    <audio ref={audioRef} controls style={{ width: "100%" }} src={resolvedSrc ?? undefined} onLoadedData={handleReady} onCanPlay={handleReady} />
  );
}
