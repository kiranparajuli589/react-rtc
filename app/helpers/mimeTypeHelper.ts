...
export function getAllSupportedMimeTypes(...mediaTypes: string[]): string[] {
  ...
  const CODECS = ["vp9", "vp8", "opus", "vorbis", "aac", "mp4a", "not-supported"];
...
export function getMostSuitableMimeType(supportedMimeTypes: string[], type: string): string | null {
  const filteredMimeTypes = supportedMimeTypes.filter((mimeType) => mimeType.startsWith(type + "/"));
