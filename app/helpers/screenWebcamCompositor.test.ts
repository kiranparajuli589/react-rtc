...
describe("screenWebcamCompositor", () => {
  it("places webcam in bottom-right by default", () => {
    ...
    expect(layout.x + layout.webcamWidth / 2).toBeGreaterThan(
      layout.canvasWidth / 2
    )
    expect(layout.y + layout.webcamHeight / 2).toBeGreaterThan(
      layout.canvasHeight / 2
    )
  })

  it("uses size fraction for overlay diameter", () => {
    const layout = computeCompositorLayout(1920, 1080, {
      centerX: 0.5,
      centerY: 0.5,
      size: "md",
    })
