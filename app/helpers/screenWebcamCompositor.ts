import { ScreenWebcamConfig, getWebcamSizeFraction } from "@/constants/recording";
import type { WebcamOverlayPlacement, WebcamOverlaySize } from "@/types/recording";
...
export const computeCompositorLayout = (
  ...
  const { x, y, width, height } = computeWebcamOverlayRect(placement, screenWidth, screenHeight);
...
export const drawScreenWebcamFrame = ({
  ...
  if (mirrorWebcam) {
    ...
    context.scale(-1, 1);
    context.drawImage(webcamVideo, canvasWidth - x - webcamWidth, y, webcamWidth, webcamHeight);
    context.restore();
  } else {
