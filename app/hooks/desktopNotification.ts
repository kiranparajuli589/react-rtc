...
export function notifyMe(message: string, title = BRAND_NAME): NotificationResult {
...
export function requestNotificationPermission(): void {
  if (!("Notification" in window)) {
    ...
    Notification.requestPermission()
      ...
      .catch((error) => {
        ConsoleLogger.warn("Failed to request permission for desktop notification", error);
      });
  }
}
...
export function buildStreamInitNotificationMap(messages: StreamInitMessages): Record<RecordingType, string> {
  return {
