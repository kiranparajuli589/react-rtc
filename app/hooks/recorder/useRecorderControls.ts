...
import { resetCanvasPrimedGate, waitForCanvasPrimed } from "@/helpers/canvasPrimeGate";
import ConsoleLogger from "@/helpers/consoleLogger";
import { getMimeDictionary } from "@/helpers/mimeTypeHelper";
import { getFileFromBlob } from "@/helpers/utils";
import { toast } from "sonner";

import { setupRecorder, stopActiveRecorder, type RecorderSetupContext } from "./useRecorderSetup";
...
export async function beginRecordingSession(ctx: RecorderControlsContext): Promise<void> {
...
export function recorderStop(ctx: RecorderControlsContext, limitReached = false): void {
  ...
  if (limitReached === true) {
    notifyMe(ctx.notifications.timeLimitReached, ctx.notifications.timeLimitTitle);
  }
}

export async function recorderStart(ctx: RecorderControlsContext, fromClick = false): Promise<void> {
  ConsoleLogger.info("on recorderStart function");
  if (ctx.withCountdown && !fromClick) return;

  const hasPriorRecording = ctx.recordedChunksRef.current.length > 0 || Boolean(ctx.recordBlob);
...
export function downloadRecording(ctx: RecorderControlsContext): void {
  ...
  const file = getFileFromBlob(ctx.recordBlob, ctx.type, MimeDictionary[ctx.type]);
