...
export function setupRecorder(ctx: RecorderSetupContext): void {
  ...
  const isScreenCapture =
    ctx.type === RECORDING_TYPE.SCREEN ||
    ctx.type === RECORDING_TYPE.SCREEN_VIDEO
  const options: MediaRecorderOptions = {
    mimeType: selectedMimeType ?? undefined,
    videoBitsPerSecond: isScreenCapture
      ? preset.screenVideoBitsPerSecond
      : preset.cameraVideoBitsPerSecond,
