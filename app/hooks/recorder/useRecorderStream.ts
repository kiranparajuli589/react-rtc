...
export default function useRecorderStream({ mediaStreamRef, setMediaStream }: UseRecorderStreamArgs) {
...
export function stopAllTracks(stream: MediaStream | null): void {
  ...
  stream.getTracks().forEach((track) => {
    track.stop();
    ConsoleLogger.info("Stopped track", { kind: track.kind, label: track.label });
  });
}
