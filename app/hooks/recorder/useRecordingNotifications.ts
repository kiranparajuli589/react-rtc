...
import {
  notifyMe,
  buildStreamInitNotificationMap,
...
export default function useRecordingNotifications({
  type,
  isRecordingRef,
}: UseRecordingNotificationsArgs) {
  const tNotifications = useTranslations("notifications")
  const intervalNotificationRef = useRef<{ close?: () => void } | null>(null)
  const niProcessTimeoutIdRef = useRef<ReturnType<typeof setInterval> | null>(
    null
  )
  ...
  const nudgeMessages = [
    tNotifications("nudgeReminder"),
    tNotifications("nudgeHello"),
  ]
  ...
  const startNotificationProcess = (): ReturnType<typeof setInterval> => {
    ...
    function notify() {
      ...
      if (messageIndex < messages.length) {
        ...
        intervalNotificationRef.current = notifyMe(message, recordingTitle) as {
          close?: () => void
        } | null
