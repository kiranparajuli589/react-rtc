...
import { calculateBarData, drawAudioVisualizer } from "@/helpers/audioVisualizer";
...
export default function useAudioVisualizer() {
  ...
  const processFrequencyData = useCallback(
    (data: Uint8Array) => {
      ...
      const dataPoints = calculateBarData(data, visualizerCanvasRef.current.width, visualizerConfig.barWidth, visualizerConfig.barGap);
      drawAudioVisualizer(
        dataPoints,
  ...
  const initAudioContext = useCallback(
    (stream: MediaStream) => {
      ...
      analyserNode.smoothingTimeConstant = visualizerConfig.smoothingTimeConstant;
