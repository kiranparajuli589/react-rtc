...
import { buildControlsContext, buildSetupContext } from "./recorder/recorderContextBuilders";
...
import { clearCanvasPrimedGate, notifyCanvasPrimed } from "@/helpers/canvasPrimeGate";
...
export default function useMediaRecorder({
  ...
  const requiresStreamRepickOnRestart = type === RECORDING_TYPE.SCREEN || type === RECORDING_TYPE.SCREEN_VIDEO;
  ...
  const {
    recordBlob, setRecordBlob, isFinalizing, setIsFinalizing, recordedChunksRef,
    activeRecorderGenerationRef, bumpGeneration, appendChunk, finalizeRecordBlob,
    resetChunks, detachRecorderHandlers,
  } = chunks;

  const { clearMediaStream } = useRecorderStream({ mediaStreamRef, setMediaStream });
  ...
  const { recordTimer, setRecordTimer, formattedTimer, resetTimer } = useRecordingTimer({
    isRecording, isPaused, onLimitReached: () => recorderStopRef.current(true),
  });

  const notifications = useRecordingNotifications({ type, isRecordingRef });
  const { startNotificationProcess, stopNotificationProcess, niProcessTimeoutIdRef } = notifications;
  ...
  const { countdownRemaining, setCountdownRemaining, countDownStatus, cancelRequestRef, startCountdown, onCancelRecordingInit, resetCountdown } = countdown;
  ...
  }, [clearMediaStream, detachRecorderHandlers, resetChunks, resetCountdown, resetTimer, stopNotificationProcess]);

  const setupCtx = buildSetupContext({
    type, mediaStreamRef, mediaRecorderRef, canvasRef, canvasSetupDeferredRef,
    isMicDisabled, canvasCaptureFrameRate, quality, recordedChunksRef, activeRecorderGenerationRef,
    isRecordingRef, pendingStopCleanupRef, bumpGeneration, appendChunk, finalizeRecordBlob,
    detachRecorderHandlers, setMediaRecorder, setIsRecording, setIsStopping, setIsPriming,
    runDeferredStreamCleanup, recorderUnsupportedMessage: tNotifications("recorderUnsupported"),
  });

  const controlsCtx = buildControlsContext({
    type, withCountdown, mediaRecorderRef, canvasRef, canvasSetupDeferredRef,
    recordedChunksRef, isRecordingRef, cancelRequestRef, isStopped, mediaRecorder, recordBlob,
    setupCtx, resetChunks, resetTimer, setIsStopped, setIsPaused, setIsPriming, setIsStopping,
    setIsFinalizing, setIsRecording, setCountdownRemaining, stopNotificationProcess, startCountdown,
  ...
  const beginRecordingSession = useCallback(async () => {
    if (controlsCtxRef.current) await beginRecordingSessionAction(controlsCtxRef.current);
  }, []);
  const recorderStop = useCallback((limitReached = false) => {
    if (controlsCtxRef.current) recorderStopAction(controlsCtxRef.current, limitReached);
  }, []);
  const recorderStart = useCallback(async (fromClick = false) => {
    if (controlsCtxRef.current) await recorderStartAction(controlsCtxRef.current, fromClick);
  }, []);
  ...
  useEffect(() => {
    ...
    if (canvasRef.current) { canvasSetupDeferredRef.current = true; return; }
  ...
  }, [mediaStream, isStopped, withCountdown, startCountdown, stopNotificationProcess]);
  ...
  }, [mediaStream, isStopped, withCountdown, startNotificationProcess]);

  return {
    mediaRecorder, setMediaRecorder, mediaStream, mediaStreamRef, setMediaStream,
    isRecording, setIsRecording, isPaused, setIsPaused, isStopped, setIsStopped,
    isPriming, isStopping, isFinalizing, isEncoderActive: isPriming || isRecording || isStopping,
    recordTimer, setRecordTimer, formattedTimer, countdownRemaining, countDownStatus,
    setCountDownStatus: setCountdownRemaining, canvasRef, recordBlob, setRecordBlob,
    cancelRequestRef, setupRecorder, clearMediaRecorder, downloadRecording,
    recorderStart, recorderStop, recorderPause, recorderResume, onCancelRecordingInit, signalCanvasPrimed,
  };
}
