...
import {
  ...
  getAudioConstraints,
  getVideoConstraints,
} from "@/helpers/mediaConstraints"
import {
  getScreenCapturePromise,
  setScreenCapturePromise,
} from "@/helpers/screenCaptureDedup"
...
export default function useStream() {
  ...
  const initAudioStream = useCallback(
    (deviceId: string | null = null, quality: Quality | null = null) => {
      ...
      ConsoleLogger.info(
        "Starting audio stream with constraints: ",
        audioConstraints
      )
  ...
  const initCameraStream = useCallback(
    (
      ...
      ConsoleLogger.info(
        "Starting camera stream with constraints: ",
        mediaConstraints
      )
      navigator.mediaDevices
        ...
        .then((stream) => {
          stream
            .getVideoTracks()
            .forEach((track) => applyContentHint(track, "detail"))
  ...
  const initScreenStream = (
    ...
    options: InitScreenStreamOptions = {}
  ) => {
    ...
    const attachScreenStream = (stream: MediaStream) => {
      stream
        .getVideoTracks()
        .forEach((track) => applyContentHint(track, "motion"))
